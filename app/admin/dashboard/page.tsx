...
export default function DashboardPage() {
  ...
  const loadData = useCallback(async () => {
    const { data: flds } = await supabase
      .from('exc_folders').select('*').order('sort_order', { ascending: true });
    setFolders(flds ?? []);
    const { data: proj } = await supabase
      .from('exc_projects').select('*').order('sort_order', { ascending: true });
    setProjects(proj ?? []);
    const { data: lds } = await supabase
      .from('exc_leads').select('*').order('created_at', { ascending: false });
    setLeads(lds ?? []);
    const { data: cmp } = await supabase
      .from('exc_comparisons').select('*').order('sort_order', { ascending: true });
    setComparisons(cmp ?? []);
  }, [supabase]);
  ...
  async function handleAddFolder(e: React.FormEvent) {
    ...
    const { error } = await supabase.from('exc_folders').insert([{
  ...
  async function saveEdit(f: Folder) {
    const { error } = await supabase.from('exc_folders').update({
  ...
  async function toggleFolder(f: Folder) {
    await supabase.from('exc_folders').update({ published: !f.published }).eq('id', f.id);
    await loadData();
  }
  ...
  async function uploadImage(f: File): Promise<string> {
    ...
    const { error } = await supabase.storage.from('excavation').upload(path, f);
    if (error) throw error;
    return supabase.storage.from('excavation').getPublicUrl(path).data.publicUrl;
  }
  ...
  async function handleAddComparison(e: React.FormEvent) {
    ...
    try {
      ...
      const { error } = await supabase.from('exc_comparisons').insert([{
  ...
  async function toggleComparison(c: Comparison) {
    await supabase.from('exc_comparisons').update({ published: !c.published }).eq('id', c.id);
    await loadData();
  }

  async function deleteComparison(c: Comparison) {
    if (!confirm('Delete this before/after comparison?')) return;
    const { error } = await supabase.from('exc_comparisons').delete().eq('id', c.id);
    if (!error) await loadData();
  }
  ...
  async function handleAddProject(e: React.FormEvent) {
    ...
    try {
      ...
      if (file) {
        ...
        const { error: upErr } = await supabase.storage.from('excavation').upload(path, file);
        if (upErr) throw upErr;
        imageUrl = supabase.storage.from('excavation').getPublicUrl(path).data.publicUrl;
      }
      const nextSort = projects.length ? Math.max(...projects.map((p) => p.sort_order)) + 1 : 1;
      const { error: insErr } = await supabase.from('exc_projects').insert([{
  ...
  async function handleDeleteProject(id: string) {
    ...
    const { error } = await supabase.from('exc_projects').delete().eq('id', id);
    if (!error) await loadData();
  }

  async function toggleProject(p: Project) {
    await supabase.from('exc_projects').update({ published: !p.published }).eq('id', p.id);
    await loadData();
  }
    ...
    <section className="section">
      <div className="container">
        ...
        <div className="card" style={{ marginBottom: 40 }}>
          ...
          <form onSubmit={handleAddFolder} style={{ borderTop: '2px solid var(--line)', marginTop: 8, paddingTop: 18 }}>
            ...
            <div style={{ marginBottom: 10 }}>
              <input style={{ width: '100%' }} value={fName} onChange={(e) => setFName(e.target.value)} placeholder="Folder name (e.g. Septic Systems)" required />
            </div>
        ...
        <div className="card" style={{ marginBottom: 40 }}>
          ...
          <p className="form-note">These appear in the &ldquo;Our Featured Projects&rdquo; slider on the home page.</p>
          ...
          <form onSubmit={handleAddComparison} style={{ borderTop: '2px solid var(--line)', marginTop: 8, paddingTop: 18 }}>
            ...
            <div className="field">
              ...
              <input id="ba-title" value={cTitle} onChange={(e) => setCTitle(e.target.value)} placeholder="e.g. Driveway grading" />
            </div>
          ...
          <form onSubmit={handleAddProject}>
            <div className="field">
              ...
              <input id="p-name" value={name} onChange={(e) => setName(e.target.value)} required
                placeholder="e.g. Septic install, Sandpoint" />
            </div>
            <div className="field">
