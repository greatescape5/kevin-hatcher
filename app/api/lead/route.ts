  ...
  if (resendKey && toEmail && fromEmail) {
    try {
      ...
      await resend.emails.send({
        ...
        text:
          `New lead from the Kevin Hatcher Excavation website:\n\n` +
          `Name: ${name}\nEmail: ${email}\nPhone: ${phone || '—'}\n` +
          `Service: ${service || '—'}\n\nMessage:\n${message || '—'}`,
      });
      ...
      await resend.emails.send({
        from: fromEmail,
        to: email,
        subject: 'We got your request — Kevin Hatcher Excavation',
        text:
          `Hi ${name},\n\n` +
          `Thanks for reaching out to Kevin Hatcher Excavation. ` +
          `We've received your request and will get back to you shortly with a free quote.\n\n` +
          `— Kevin Hatcher Excavation`,
      });
    } catch {
